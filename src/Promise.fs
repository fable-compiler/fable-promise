...
type PromiseBuilder() =
    member _.Bind(p: JS.Promise<'T1>, f: 'T1 -> JS.Promise<'T2>): JS.Promise<'T2> =
        p |> bind f

    member _.Combine(p1: JS.Promise<unit>, p2: unit -> JS.Promise<'T>): JS.Promise<'T> =
        p1 |> bind p2
    ...
    member _.For(p: JS.Promise<'T1>, f: 'T1 -> JS.Promise<'T2>): JS.Promise<'T2> =
        p |> bind f

    member this.While(guard: unit -> bool, p: unit -> JS.Promise<unit>): JS.Promise<unit> =
        if guard()
        then p() |> bind (fun () -> this.While(guard, p))
        else lift()

    member _.Return(a: 'T): JS.Promise<'T> = lift a

    member _.ReturnFrom(p: JS.Promise<'T>): JS.Promise<'T> = p

    member _.Zero(): JS.Promise<unit> = lift ()

    member _.TryFinally(p: unit -> JS.Promise<'T>, compensation: unit -> unit): JS.Promise<'T> =
        try
            p() |> either
                (fun (x: 'T) -> compensation(); x)
                (fun er -> compensation(); raise er)
        with er ->
            compensation()
            reject er

    member _.TryWith(p: unit -> JS.Promise<'T>, catchHandler: exn -> JS.Promise<'T>): JS.Promise<'T> =
        try
            p() |> catchBind catchHandler
        with er ->
            catchHandler er

    member _.Delay(generator: unit -> JS.Promise<'T>): unit -> JS.Promise<'T> =
        generator

    member _.Run(generator: unit -> JS.Promise<'T>): JS.Promise<'T> =
        try
            generator()
        with er ->
            reject er
    ...
    member this.Using<'T1, 'T2 when 'T1 :> IDisposable>(resource: 'T1, binder: 'T1 -> JS.Promise<'T2>): JS.Promise<'T2> =
        this.TryFinally((fun () -> binder(resource)), (fun () -> resource.Dispose()))
